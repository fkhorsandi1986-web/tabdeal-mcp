...
def test_build_target_creates_short_with_downside_targets():
    result = build_target(base_item(imbalance=-0.30, shift=-0.03))
    assert result is not None
...
def test_build_targets_prefers_tighter_spread_when_strength_ties():
    wide = base_item(symbol="WIDE", spread_pct=0.80)
    tight = base_item(symbol="TIGHT", spread_pct=0.10)
    results = build_targets([wide, tight], limit=2)
    assert results[0]["symbol"] == "TIGHT"
    assert results[1]["symbol"] == "WIDE"
